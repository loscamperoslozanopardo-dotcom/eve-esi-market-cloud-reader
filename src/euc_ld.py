...
def fetch_systems_list(
    ...
    """Fetch /universe/systems/ with ETag/Last-Modified conditional headers."""
...
def fetch_system_info(
    ...
    """Fetch /universe/systems/{system_id}/ (body always needed; do NOT use 304)."""
    ...
    if status != 200 or not isinstance(data, dict):
        raise RuntimeError(
            f"Unexpected system info response for {system_id}: status={status}"
        )
    ...
    try:
        constellation_id_int = int(constellation_id)
    except Exception:
        raise RuntimeError(
            f"Missing/invalid 'constellation_id' for system {system_id}"
        ) from None

    return {
        "system_id": int(system_id),
        "system_name": name,
        "constellation_id": constellation_id_int,
    }
...
def main() -> int:
    ...
    # 1) Fetch system IDs list (conditional with ETag / Last-Modified)
    ...
    new_state["last_modified"] = (
        ...
        or new_state.get("last_modified")
    )
