...
CONSTELLATION_INFO_PATH_TMPL = "/universe/constellations/{constellation_id}/"
...
def load_constellations_to_bigquery_from_list(
    project_id: str,
    dataset_id: str,
    table_id: str,
    ...
    job_config = bigquery.LoadJobConfig(
        schema=[
            ...
            bigquery.SchemaField("constellation_name", "STRING", mode="REQUIRED"),
...
def write_constellations_dt_table(
    project_id: str,
    dataset_id: str,
    table_id: str,
    expires_dt: datetime,
...
def fetch_constellation_info(
    ...
    if status != 200 or not isinstance(data, dict):
        raise RuntimeError(
            f"Unexpected constellation info response for {constellation_id}: status={status}"
        )
...
class RunConfig:
    ...
    dataset_id: str
    location: str
    table_constellations: str
    table_constellations_dt: str
...
def main() -> int:
    ...
    table_constellations = os.getenv("TABLE_CONSTELLATIONS", "constellations")
    table_constellations_dt = os.getenv("TABLE_CONSTELLATIONS_DT", "constellations_dt")
    ...
    cfg = RunConfig(
        ...
        location=location,
        table_constellations=table_constellations,
        table_constellations_dt=table_constellations_dt,
        esi_base=esi_base,
    ...
    if status not in (200, 304):
        raise RuntimeError(f"Unexpected ESI status for constellations list: {status}")
    ...
    write_constellations_dt_table(
        ...
        table_id=cfg.table_constellations_dt,
    ...
    if status == 200:
        ...
        for x in data:
            try:
                constellation_ids.append(int(x))
            except Exception:
                continue

        if not constellation_ids:
            raise RuntimeError("ESI constellations list returned no valid constellation IDs")

        from concurrent.futures import ThreadPoolExecutor, as_completed

        max_workers = min(cfg.workers, len(constellation_ids))
        rows: List[Dict[str, Any]] = []
        errors: List[str] = []
        ...
        loaded_rows = load_constellations_to_bigquery_from_list(
            project_id=cfg.project_id,
            dataset_id=cfg.dataset_id,
            table_id=cfg.table_constellations,
            rows=rows,
            location=cfg.location,
        )
