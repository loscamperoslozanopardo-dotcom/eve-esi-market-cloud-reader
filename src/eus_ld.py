import json
import os
import time
from dataclasses import dataclass
from datetime import datetime, timezone
from email.utils import parsedate_to_datetime
from io import BytesIO
from typing import Any, Dict, List, Optional, Tuple

import requests
from google.cloud import bigquery


# -----------------------------
# ESI endpoints (Solar systems)
# -----------------------------
ESI_BASE_DEFAULT = "https://esi.evetech.net/latest"
SYSTEMS_LIST_PATH = "/universe/systems/"
SYSTEM_INFO_PATH_TMPL = "/universe/systems/{system_id}/"


# -----------------------------
# Small utils
# -----------------------------
def now_utc() -> datetime:
    return datetime.now(timezone.utc)


def iso_z(dt: datetime) -> str:
    dt = dt.astimezone(timezone.utc)
    return dt.replace(microsecond=0).isoformat().replace("+00:00", "Z")


def parse_http_date(value: Optional[str]) -> Optional[datetime]:
    if not value or not isinstance(value, str):
        return None
    try:
        dt = parsedate_to_datetime(value)
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=timezone.utc)
        return dt.astimezone(timezone.utc)
    except Exception:
        return None


def env_int(name: str, default: int) -> int:
    raw = os.getenv(name)
    if raw is None:
        return default
    try:
        return int(str(raw).strip())
    except Exception:
        return default


def env_bool(name: str, default: bool) -> bool:
    raw = os.getenv(name)
    if raw is None:
        return default
    raw = str(raw).strip().lower()
    if raw in ("1", "true", "t", "yes", "y", "on"):
        return True
    if raw in ("0", "false", "f", "no", "n", "off"):
        return False
    return default


def load_json(path: str, default: Any) -> Any:
    try:
        with open(path, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return default


def save_json_atomic(path: str, obj: Any) -> None:
    os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    tmp = f"{path}.tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(obj, f, ensure_ascii=False, indent=2, sort_keys=True)
        f.write("\n")
    os.replace(tmp, path)


# -----------------------------
# BigQuery helpers
# -----------------------------
def _bq_dataset_ref(client: bigquery.Client, project_id: str, dataset_id: str) -> bigquery.DatasetReference:
    return bigquery.DatasetReference(project_id, dataset_id)


def ensure_dataset(project_id: str, dataset_id: str, location: str) -> None:
    client = bigquery.Client(project=project_id)
    ds_ref = _bq_dataset_ref(client, project_id, dataset_id)
    try:
        client.get_dataset(ds_ref)
        return
    except Exception:
        ds = bigquery.Dataset(ds_ref)
        ds.location = location
        client.create_dataset(ds, exists_ok=True)


def write_systems_dt_table(
    project_id: str,
    dataset_id: str,
    table_id: str,
    expires_dt: datetime,
    location: str,
) -> None:
    """
    Creates/overwrites systems_dt with 1 row, 1 column:
      - expires_utc TIMESTAMP
    """
    client = bigquery.Client(project=project_id, location=location)
    table_fq = f"{project_id}.{dataset_id}.{table_id}"

    schema = [bigquery.SchemaField("expires_utc", "TIMESTAMP", mode="REQUIRED")]

    job_config = bigquery.LoadJobConfig(
        schema=schema,
        source_format=bigquery.SourceFormat.NEWLINE_DELIMITED_JSON,
        write_disposition=bigquery.WriteDisposition.WRITE_TRUNCATE,
        create_disposition=bigquery.CreateDisposition.CREATE_IF_NEEDED,
    )

    payload = json.dumps({"expires_utc": iso_z(expires_dt)}, ensure_ascii=False) + "\n"
    bio = BytesIO(payload.encode("utf-8"))

    job = client.load_table_from_file(bio, table_fq, job_config=job_config)
    job.result()  # wait


def load_systems_to_bigquery_from_list(
    project_id: str,
    dataset_id: str,
    table_id: str,
    rows: List[Dict[str, Any]],
    location: str,
) -> int:
    """
    Replaces systems table with the given rows.
    Schema:
      - system_id INT64
      - system_name STRING
      - constellation_id INT64
      - position_x FLOAT64
      - position_y FLOAT64
      - position_z FLOAT64
      - security_status FLOAT64
    """
    client = bigquery.Client(project=project_id, location=location)
    table_fq = f"{project_id}.{dataset_id}.{table_id}"

    schema = [
        bigquery.SchemaField("system_id", "INT64", mode="REQUIRED"),
        bigquery.SchemaField("system_name", "STRING", mode="REQUIRED"),
        bigquery.SchemaField("constellation_id", "INT64", mode="REQUIRED"),
        bigquery.SchemaField("position_x", "FLOAT64", mode="REQUIRED"),
        bigquery.SchemaField("position_y", "FLOAT64", mode="REQUIRED"),
        bigquery.SchemaField("position_z", "FLOAT64", mode="REQUIRED"),
        bigquery.SchemaField("security_status", "FLOAT64", mode="REQUIRED"),
    ]

    job_config = bigquery.LoadJobConfig(
        schema=schema,
        source_format=bigquery.SourceFormat.NEWLINE_DELIMITED_JSON,
        write_disposition=bigquery.WriteDisposition.WRITE_TRUNCATE,
        create_disposition=bigquery.CreateDisposition.CREATE_IF_NEEDED,
        ignore_unknown_values=True,
    )

    ndjson = "\n".join(json.dumps(r, ensure_ascii=False) for r in rows) + "\n"
    bio = BytesIO(ndjson.encode("utf-8"))

    job = client.load_table_from_file(bio, table_fq, job_config=job_config)
    job.result()
    return len(rows)


# -----------------------------
# ESI request layer
# -----------------------------
def _headers_lower(d: Dict[str, str]) -> Dict[str, str]:
    return {str(k).lower(): str(v) for k, v in d.items()}


def _esi_get(
    url: str,
    params: Dict[str, Any],
    headers: Optional[Dict[str, str]],
    user_agent: str,
    timeout_s: int,
    max_retries: int,
    error_limit_threshold: int,
) -> Tuple[int, Dict[str, str], Any]:
    """
    GET with:
      - backoff retries on 429/502/503/504 + network errors
      - respects ESI error-limit headers
      - handles 420 (Error limited) by sleeping reset seconds, then retry
    """
    sess = requests.Session()

    base_headers = {
        "Accept": "application/json",
        "Accept-Encoding": "gzip",
        "User-Agent": user_agent,
        "X-User-Agent": user_agent,
    }
    if headers:
        base_headers.update(headers)

    last_exc: Optional[Exception] = None

    for attempt in range(1, max_retries + 1):
        try:
            resp = sess.get(url, params=params, headers=base_headers, timeout=timeout_s)

            hdrs = dict(resp.headers or {})
            hdrs_l = _headers_lower(hdrs)

            # Error limit protection (best-effort)
            remain = hdrs_l.get("x-esi-error-limit-remain")
            reset = hdrs_l.get("x-esi-error-limit-reset")
            try:
                remain_i = int(remain) if remain is not None else None
                reset_i = int(reset) if reset is not None else None
            except Exception:
                remain_i = None
                reset_i = None

            if remain_i is not None and reset_i is not None and remain_i <= error_limit_threshold:
                time.sleep(min(max(reset_i, 1), 60))

            # 420: error-limited
            if resp.status_code == 420:
                reset_s = 60
                try:
                    reset_s = int(hdrs_l.get("x-esi-error-limit-reset", "60"))
                except Exception:
                    reset_s = 60
                if attempt < max_retries:
                    time.sleep(min(max(reset_s, 1), 300) + 1)
                    continue
                return resp.status_code, hdrs, None

            # 429 & transient server errors
            if resp.status_code in (429, 502, 503, 504):
                if attempt < max_retries:
                    retry_after = hdrs_l.get("retry-after")
                    if retry_after is not None:
                        try:
                            sleep_s = int(float(retry_after))
                        except Exception:
                            sleep_s = 2 ** (attempt - 1)
                    else:
                        sleep_s = 2 ** (attempt - 1)
                    time.sleep(min(max(sleep_s, 1), 120))
                    continue
                return resp.status_code, hdrs, None

            data: Any = None
            if resp.status_code != 304:
                if resp.content:
                    try:
                        data = resp.json()
                    except Exception:
                        data = None

            return resp.status_code, hdrs, data

        except (requests.Timeout, requests.ConnectionError, requests.RequestException) as e:
            last_exc = e
            if attempt < max_retries:
                time.sleep(min(2 ** (attempt - 1), 60))
                continue
            raise

    raise last_exc or RuntimeError("Unexpected retry loop exit")


def fetch_systems_list(
    prev_state: Dict[str, Any],
    esi_base: str,
    datasource: str,
    user_agent: str,
    timeout_s: int,
    max_retries: int,
    error_limit_threshold: int,
) -> Dict[str, Any]:
    """
    Fetch /universe/systems/ with conditional headers:
      - If-None-Match from previous ETag
      - If-Modified-Since from previous Last-Modified
    """
    url = f"{esi_base}{SYSTEMS_LIST_PATH}"

    cond: Dict[str, str] = {}
    prev_etag = prev_state.get("etag")
    if isinstance(prev_etag, str) and prev_etag:
        cond["If-None-Match"] = prev_etag
    prev_lm = prev_state.get("last_modified")
    if isinstance(prev_lm, str) and prev_lm:
        cond["If-Modified-Since"] = prev_lm

    status, hdrs, data = _esi_get(
        url=url,
        params={"datasource": datasource},
        headers=cond,
        user_agent=user_agent,
        timeout_s=timeout_s,
        max_retries=max_retries,
        error_limit_threshold=error_limit_threshold,
    )

    return {"status": status, "headers": hdrs, "data": data}


def fetch_system_info(
    system_id: int,
    esi_base: str,
    datasource: str,
    language: str,
    user_agent: str,
    timeout_s: int,
    max_retries: int,
    error_limit_threshold: int,
) -> Dict[str, Any]:
    """
    Fetch /universe/systems/{system_id}/.
    Extract required fields for BigQuery row.
    """
    url = f"{esi_base}{SYSTEM_INFO_PATH_TMPL.format(system_id=system_id)}"

    status, _hdrs, data = _esi_get(
        url=url,
        params={"datasource": datasource, "language": language},
        headers=None,
        user_agent=user_agent,
        timeout_s=timeout_s,
        max_retries=max_retries,
        error_limit_threshold=error_limit_threshold,
    )

    if status != 200 or not isinstance(data, dict):
        raise RuntimeError(f"Unexpected system info response for {system_id}: status={status}")

    name = data.get("name")
    if not isinstance(name, str) or not name:
        raise RuntimeError(f"Missing/invalid 'name' for system {system_id}")

    constellation_id = data.get("constellation_id")
    if constellation_id is None:
        raise RuntimeError(f"Missing 'constellation_id' for system {system_id}")

    pos = data.get("position")
    if not isinstance(pos, dict):
        raise RuntimeError(f"Missing/invalid 'position' for system {system_id}")

    def _f(v: Any, field: str) -> float:
        try:
            return float(v)
        except Exception:
            raise RuntimeError(f"Missing/invalid '{field}' for system {system_id}")

    position_x = _f(pos.get("x"), "position.x")
    position_y = _f(pos.get("y"), "position.y")
    position_z = _f(pos.get("z"), "position.z")

    security_status = _f(data.get("security_status"), "security_status")

    return {
        "system_id": int(system_id),
        "system_name": name,
        "constellation_id": int(constellation_id),
        "position_x": position_x,
        "position_y": position_y,
        "position_z": position_z,
        "security_status": security_status,
    }


# -----------------------------
# Main
# -----------------------------
@dataclass(frozen=True)
class RunConfig:
    project_id: str
    dataset_id: str
    location: str
    table_systems: str
    table_systems_dt: str
    esi_base: str
    datasource: str
    language: str
    user_agent: str
    state_file: str
    force_verify_after_expires: bool
    timeout_s: int
    max_retries: int
    error_limit_threshold: int
    workers: int


def _load_state(path: str) -> Dict[str, Any]:
    state = load_json(path, {}) if path else {}
    return state if isinstance(state, dict) else {}


def main() -> int:
    # ---- env (BQ)
    project_id = os.getenv("PROJECT_ID", "eve-market-sandbox")
    location = os.getenv("LOCATION", "EU")
    dataset_id = os.getenv("DATASET_ID", "eve_market")
    table_systems = os.getenv("TABLE_SYSTEMS", "systems")
    table_systems_dt = os.getenv("TABLE_SYSTEMS_DT", "systems_dt")

    # ---- env (ESI)
    esi_base = os.getenv("ESI_BASE", ESI_BASE_DEFAULT)
    datasource = os.getenv("ESI_DATASOURCE", "tranquility")
    language = os.getenv("ESI_LANGUAGE", "en")
    user_agent = os.getenv("USER_AGENT", "eve-esi-market-cloud-reader (github actions)")

    # ---- env (policy)
    state_file = os.getenv("STATE_FILE", os.path.join("state", "eus_hdrs.json"))
    force_verify_after_expires = env_bool("FORCE_VERIFY_AFTER_EXPIRES", True)

    # ---- env (robust)
    timeout_s = env_int("TIMEOUT_S", 60)
    max_retries = env_int("MAX_RETRIES", 5)
    error_limit_threshold = env_int("ERROR_LIMIT_THRESHOLD", 20)
    workers = env_int("WORKERS", 12)
    workers = max(1, min(workers, 25))

    cfg = RunConfig(
        project_id=project_id,
        dataset_id=dataset_id,
        location=location,
        table_systems=table_systems,
        table_systems_dt=table_systems_dt,
        esi_base=esi_base,
        datasource=datasource,
        language=language,
        user_agent=user_agent,
        state_file=state_file,
        force_verify_after_expires=force_verify_after_expires,
        timeout_s=timeout_s,
        max_retries=max_retries,
        error_limit_threshold=error_limit_threshold,
        workers=workers,
    )

    state = _load_state(cfg.state_file)

    # Respetar Expires (no pedir antes de tiempo)
    prev_expires = state.get("expires")
    prev_expires_dt = parse_http_date(prev_expires) if isinstance(prev_expires, str) else None
    now = now_utc()
    if cfg.force_verify_after_expires and prev_expires_dt and now < prev_expires_dt:
        print(json.dumps({"note": "skip_not_expired", "expires_utc": iso_z(prev_expires_dt)}, ensure_ascii=False))
        return 0

    # 1) Fetch systems list (conditional)
    res = fetch_systems_list(
        prev_state=state,
        esi_base=cfg.esi_base,
        datasource=cfg.datasource,
        user_agent=cfg.user_agent,
        timeout_s=cfg.timeout_s,
        max_retries=cfg.max_retries,
        error_limit_threshold=cfg.error_limit_threshold,
    )

    status = int(res["status"])
    headers = res["headers"]
    data = res["data"]

    if status not in (200, 304):
        raise RuntimeError(f"Unexpected ESI status for systems list: {status}")

    expires = headers.get("Expires") or headers.get("expires")
    expires_dt = parse_http_date(expires)
    if not expires_dt:
        raise RuntimeError("Missing/invalid Expires header on successful response; cannot write systems_dt")

    request_utc = now_utc()

    # 2) BigQuery: ensure dataset + write dt table (si ESI fue OK)
    ensure_dataset(project_id=cfg.project_id, dataset_id=cfg.dataset_id, location=cfg.location)
    write_systems_dt_table(
        project_id=cfg.project_id,
        dataset_id=cfg.dataset_id,
        table_id=cfg.table_systems_dt,
        expires_dt=expires_dt,
        location=cfg.location,
    )

    loaded_rows: Optional[int] = None

    # 3) Si cambió (200), cargar systems (tabla completa)
    if status == 200:
        if not isinstance(data, list):
            raise RuntimeError("ESI systems list payload is not a list")

        system_ids: List[int] = []
        for x in data:
            try:
                system_ids.append(int(x))
            except Exception:
                continue

        if not system_ids:
            raise RuntimeError("ESI systems list returned no valid system IDs")

        from concurrent.futures import ThreadPoolExecutor, as_completed

        max_workers = min(cfg.workers, len(system_ids))
        rows: List[Dict[str, Any]] = []
        errors: List[str] = []

        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            futs = [
                ex.submit(
                    fetch_system_info,
                    sid,
                    cfg.esi_base,
                    cfg.datasource,
                    cfg.language,
                    cfg.user_agent,
                    cfg.timeout_s,
                    cfg.max_retries,
                    cfg.error_limit_threshold,
                )
                for sid in system_ids
            ]

            for fut in as_completed(futs):
                try:
                    rows.append(fut.result())
                except Exception as e:
                    errors.append(str(e))

        if errors:
            raise RuntimeError(f"Failed to fetch {len(errors)} system(s). Example: {errors[0]}")

        rows.sort(key=lambda r: int(r["system_id"]))

        loaded_rows = load_systems_to_bigquery_from_list(
            project_id=cfg.project_id,
            dataset_id=cfg.dataset_id,
            table_id=cfg.table_systems,
            rows=rows,
            location=cfg.location,
        )

    # 4) Persistir estado SOLO tras éxito end-to-end (ESI OK + BQ OK)
    new_state = dict(state)
    new_state["etag"] = headers.get("ETag") or headers.get("etag") or new_state.get("etag")
    new_state["last_modified"] = (
        headers.get("Last-Modified") or headers.get("last-modified") or new_state.get("last_modified")
    )
    new_state["expires"] = expires
    new_state["last_success_request_utc"] = iso_z(request_utc)
    new_state["last_success_http_status"] = status
    new_state["last_success_expires_utc"] = iso_z(expires_dt)
    if loaded_rows is not None:
        new_state["last_success_loaded_rows"] = loaded_rows

    if cfg.state_file:
        save_json_atomic(cfg.state_file, new_state)

    print(
        json.dumps(
            {
                "status": status,
                "request_utc": iso_z(request_utc),
                "expires_utc": iso_z(expires_dt),
                "loaded_rows": loaded_rows,
                "note": "bq_loaded" if status == 200 else "not_modified_304",
            },
            ensure_ascii=False,
        )
    )

    return 0


if __name__ == "__main__":
    try:
        raise SystemExit(main())
    except Exception as e:
        # El workflow gestiona los reintentos 10 min.
        print(f"::error::{type(e).__name__}: {e}")
        raise
